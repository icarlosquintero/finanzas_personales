...
export default function Config() {
  ...
  // Keyword rules state
  const [keywordRules, setKeywordRules] = useState([]) // [{ keyword, category }]
  ...
  const handleAddKeywordRule = async (e) => {
    e.preventDefault()
    if (!newKeyword.trim() || !newKeywordCategory.trim()) return
    const keyword = newKeyword.trim().toLowerCase()
    // Avoid duplicates
    if (keywordRules.some(r => r.keyword.toLowerCase() === keyword)) return
    const updated = [...keywordRules, { keyword, category: newKeywordCategory }]
    setKeywordRules(updated)
    const updatedSettings = { ...settings, keywordRules: updated }
    setSettings(updatedSettings)
    await saveSettings(updatedSettings)
    setNewKeyword('')
  }

  const handleDeleteKeywordRule = async (keyword) => {
    const updated = keywordRules.filter(r => r.keyword !== keyword)
    setKeywordRules(updated)
    const updatedSettings = { ...settings, keywordRules: updated }
    setSettings(updatedSettings)
    await saveSettings(updatedSettings)
  ...
  const handleExport = async () => {
    try {
      const { getAllTransactions, getAccounts, getRecurring, getDebts, getSettings } = await import('@/lib/db')
      const { supabase } = await import('@/lib/supabase')
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) { alert('Debes iniciar sesión para exportar.'); return }
      const userId = user.id

      // Fetch budgets directly (no exported helper wraps all months)
      const { data: budgetRows } = await supabase.from('budgets').select('*').eq('user_id', userId)
      const { data: catRow } = await supabase.from('categories').select('list').eq('user_id', userId).single()

      const [txs, accs, recs, debts, sets] = await Promise.all([
        getAllTransactions(),
        getAccounts(),
        getRecurring(),
        getDebts(),
      ...
      <div className="container">
        ...
        <div className="tabs mb-6" style={{ display: 'flex', gap: '8px', borderBottom: '1px solid var(--color-border)', paddingBottom: '8px' }}>
          ...
          <button 
            ...
            style={{ background: activeTab === 'keywords' ? 'var(--bg-secondary)' : 'none', border: 'none', cursor: 'pointer' }}
          >
            Palabras Clave
          </button>
          <button 
        ...
        {/* TAB 2: PALABRAS CLAVE */}
        {activeTab === 'keywords' && (
          <div className="card animate-fadeIn" style={{ padding: '24px' }}>
            <h2 className="mb-2" style={{ fontSize: '1.25rem' }}>Palabras Clave → Categoría</h2>
            <p className="text-secondary text-sm mb-6">
              Cuando escribas una palabra clave en la descripción de un gasto, la categoría se autocompletará automáticamente. Las reglas son insensibles a mayúsculas.
            </p>

            {/* Add rule form */}
            <form onSubmit={handleAddKeywordRule} className="mb-6" style={{ borderBottom: '1px solid var(--color-border)', paddingBottom: '20px' }}>
              <h3 className="mb-3 text-sm font-bold">Agregar Regla</h3>
              <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
                <div className="form-field" style={{ flex: '1 1 180px' }}>
                  ...
                  <input
                    type="text"
                    className="input"
                    placeholder="ej: uber, rappi, netflix"
                    value={newKeyword}
            ...
            {/* Rules list */}
            <div>
              <h3 className="mb-3 text-sm font-bold">Reglas activas ({keywordRules.length})</h3>
              {keywordRules.length === 0 ? (
                <p className="text-secondary text-center py-6">No hay palabras clave configuradas.</p>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {[...keywordRules].sort((a, b) => a.keyword.localeCompare(b.keyword, 'es')).map(rule => (
                    <div
                      key={rule.keyword}
                      className="card"
                      style={{
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                        <span style={{ fontWeight: 700, fontFamily: 'monospace', fontSize: '0.95rem', color: 'var(--color-accent)' }}>
                          {rule.keyword}
                        </span>
                        <span style={{ color: 'var(--color-text-secondary)', fontSize: '0.85rem' }}>→</span>
                        <span style={{ fontWeight: 600, fontSize: '0.9rem' }}>{rule.category}</span>
                      </div>
                      <button
                        onClick={() => handleDeleteKeywordRule(rule.keyword)}
                        title="Eliminar regla"
                        style={{
                          display: 'flex', alignItems: 'center', background: 'none', border: 'none',
                          padding: 0, cursor: 'pointer', color: 'var(--color-danger)', flexShrink: 0
                        }}
                      >
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                          <polyline points="3 6 5 6 21 6"></polyline>
